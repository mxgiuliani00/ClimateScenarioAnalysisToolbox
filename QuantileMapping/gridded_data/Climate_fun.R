...
nc_from_ts <- function(x, init, var, id, pss, init_n) {
  ...
  for (r in 1:nR) {
    for (c in 1:nC) {
      ...
      if (idx) {
        ...
        nc_gen(Dc, var, 'control', id[c], lat, lon, init_n)
        nc_gen(Df, var, pss[r], id[c], lat, lon, init_n)
      }
...
nc_gen <- function(z, var, label_x, label_y, lat, lon, init_n) {
